...
verifyFields = (requiredFields, req) => {
    for(let i=0; i < requiredFields.length;i++){
      let field = requiredFields[i];
      if ( !(field in req.body)) {
        return { ok : false, which : field };
      }
    }
...
app.post('/shopping-list', jsonParser, (req, res) => {
  ...
  let fieldValidation = verifyFields(requiredFields, req);
  if ( !fieldValidation.ok ) {
      let message = `Missing \`${fieldValidation.which}\' in request body`;
